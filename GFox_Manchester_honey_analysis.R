...
setwd("~/Dropbox/PDRA_19-21/Manc_honey_QIIME2/github_test/Mancester_honey/")
...
## secondly remove low frequency (<1%) OTUs within a sample
...
#write.table(file="all_rbcL_fam_data.txt", rbcL_dada2_fam[which(rowSums(rbcL_dada2_fam[9:22])>0),], sep=",", quote=FALSE)
#write.table(file="all_rbcL_gen_data.txt", rbcL_dada2_gen[which(rowSums(rbcL_dada2_gen[11:24])>0),], sep=",", quote=FALSE)
#write.table(file="all_rbcL_spec_data.txt", rbcL_dada2_spec[which(rowSums(rbcL_dada2_spec[13:26])>0),], sep=",", quote=FALSE)
#write.table(file="all_ITS2p_fam_data.txt", ITS2p_dada2_fam[which(rowSums(ITS2p_dada2_fam[9:22])>0),], sep=",", quote=FALSE)
#write.table(file="all_ITS2p_gen_data.txt", ITS2p_dada2_gen[which(rowSums(ITS2p_dada2_gen[11:24])>0),], sep=",", quote=FALSE)
#write.table(file="all_ITS2p_spec_data.txt", ITS2p_dada2_spec[which(rowSums(ITS2p_dada2_spec[13:26])>0),], sep=",", quote=FALSE)
...
hive_shan_vs_5000m_shan_plot <- ggscatter(hive_shan_vs_5000m_shan, y="V1", x="V2", add="reg.line", xlab="Land Cover Shannon Diversity (5000m)", 
                                          ylab="Plant Taxa Shannon Diversity", add.params=list(color="blue", fill="darkgray"), conf.int=T, label=hive_names, repel=T)
corr.test(hive_shan_vs_5000m_shan)

## add in R^2 values of the Pearson's calculated above, and adjust p-values by the BH method
annotation_text <- grobTree(textGrob("R^2 = 0.66, p=0.002", x=0.4,  y=0.85, hjust=0,
...
for (landtype in restricted_land_cover_abbrvs){
  ...
  for (buffer in restr_buffers[, grep(paste(landtype,"_Prop", sep=""), colnames(restr_buffers))]){
    count = count+1
    temp <- as.data.frame(cbind(LandDiv$Hive_Tots, buffer))
