...
## correlation between the reads and counts of those species in the overlap?
overlap_reads <- cbind(t(rbcL_dada2_spec[which(rbcL_dada2_spec$Species=="Impatiens glandulifera"),][13:26]),
  t(ITS2p_dada2_spec[which(ITS2p_dada2_spec$Species=="Impatiens glandulifera"),][13:26]),
  t(rbcL_dada2_spec[which(rbcL_dada2_spec$Species=="Trifolium repens"),][13:26]),
  t(ITS2p_dada2_spec[which(ITS2p_dada2_spec$Species=="Trifolium repens"),][13:26])
  )
colnames(overlap_reads) <- c("Ig_rbcL_reads", "Ig_ITS2p_reads", "Tr_rbcL_reads", "Tr_ITS2p_reads")
overlap_reads <- as.data.frame(overlap_reads)

## test for correlation between number of reads assigned to each common species
print(corr.test(overlap_reads, 
                method="pearson", use="pairwise", adjust="BY", alpha=.05), short=F)
...
for (landtype in restricted_land_cover_abbrvs){
  ...
  for (buffer in restr_buffers[, grep(paste(landtype,"_Prop", sep=""), colnames(restr_buffers))]){
    ...
    stat_cor(aes(label = "blah"), method="pearson", label.x=3)
